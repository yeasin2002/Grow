...
const itemTransition = LinearTransition.duration(180).easing(Easing.out(Easing.cubic));
...
export function BottomNavigation() {
  ...
  function handleActionPress(route: (typeof ACTION_NAVIGATION_ITEMS)[number]["route"]) {
  ...
  return (
    <View className="px-3.5" style={{ paddingBottom: Math.max(insets.bottom, 14) }}>
      <View className="flex-row items-center gap-3.5">
        ...
        <View className="relative ml-auto w-12 items-center self-end">
          ...
          >
            <Icons className="text-white" name={isActionMenuOpen ? "close" : "grid"} size={20} />
          </AnimatedPressable>
        </View>
      </View>
    </View>
