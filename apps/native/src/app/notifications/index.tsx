      ...
      >
        ...
        {notificationSections.map((section) => (
          <View key={section.id} className="mb-6">
            <Text className="mb-4 text-[16px] font-medium text-[#8E8E93]">{section.title}</Text>
            ...
            {section.items.map((item, index) => {
              ...
              return (
                <View
                  key={item.id}
                  className={!isLastItem ? "border-b border-[#EFEFEF]" : undefined}
                >
                  <View className="flex-row items-start gap-3 py-5">
                    <View className="mt-1 h-12 w-12 items-center justify-center rounded-full bg-[#F4F4F4]">
                      <Icons className="text-[#111111]" name={item.icon} size={22} />
                    </View>
                    ...
                    <View className="flex-1 pr-2">
                      ...
                      <Text className="mt-2 text-[14px] text-[#9B9B9B]">{item.time}</Text>
                    </View>
                    ...
                    >
                      <Icons className="text-[#555555]" name="ellipsis-vertical" size={18} />
                    </Pressable>
                  </View>
                </View>
              );
