...
function createCalendarRows(viewMonth: Date) {
  ...
  for (let rowIndex = 0; rowIndex < 6; rowIndex += 1) {
    ...
    for (let columnIndex = 0; columnIndex < WEEK_DAYS.length; columnIndex += 1) {
...
function MonthlyCalendar({ initialDate, onDateChange }: MonthlyCalendarProps) {
  const resolvedInitialDate = initialDate ?? new Date();
  const [visibleMonth, setVisibleMonth] = useState(() => startOfMonth(resolvedInitialDate));
  ...
  return (
    <View className="px-3 pt-3">
      <View className="rounded-4xl bg-[#F2F2F2] px-3.5 py-4" style={CARD_SHADOW}>
        <View className="flex-row items-center justify-between">
          ...
          <View className="items-center">
            ...
            <View className="mt-1.5 flex-row items-center gap-2">
              ...
              >
                <Icons className="text-[#7A7A7A]" name="chevron-back" size={16} />
              </Pressable>

              <Text className="text-[14px] font-medium text-[#8C8C8C]">{year}</Text>
              ...
              >
                <Icons className="text-[#7A7A7A]" name="chevron-forward" size={16} />
              </Pressable>
            </View>
          </View>
          ...
          <Pressable
            ...
            style={CARD_SHADOW}
          >
            <Icons className="text-[#1D1D1D]" name="chevron-forward" size={20} />
          </Pressable>
        </View>
                      ...
                      >
                        <Text
                          className={cn(
                            ...
                            isCurrentMonth ? "text-[#7B7B7B]" : "text-[#B3B3B3]",
