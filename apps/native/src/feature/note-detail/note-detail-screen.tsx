...
import { NoteDetailActionBar } from "./note-detail-action-bar";
...
export function NoteDetailScreen() {
  ...
  const title = typeof params.id === "string" ? params.id : "Grocery Listing...";
      ...
      >
        ...
        <View className="mt-32">
          {CHECKLIST_ITEMS.map((item) => (
            <ChecklistItem key={item.id} completed={item.completed} label={item.label} />
          ))}
        </View>
